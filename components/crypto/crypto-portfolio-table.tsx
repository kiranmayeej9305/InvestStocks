...
export function CryptoPortfolioTable({ holdings, onRefresh, onDelete }: CryptoPortfolioTableProps) {
  const [deleteDialog, setDeleteDialog] = useState<{ open: boolean; holding: CryptoHolding | null }>({
    open: false,
    holding: null,
  })
  const [notesDialog, setNotesDialog] = useState<{ open: boolean; holding: CryptoHolding | null }>({
    open: false,
    holding: null,
  })
    ...
    <Card className="bg-white/70 dark:bg-slate-800/40 backdrop-blur-xl border-gray-200/50 dark:border-slate-700/50">
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="text-xl font-bold text-foreground">My Crypto Holdings</CardTitle>
            <p className="text-sm text-muted-foreground mt-1">
              {holdings.length} {holdings.length === 1 ? 'coin' : 'coins'} tracked
            </p>
          </div>
          <AddCryptoDialog onSuccess={onRefresh} />
        </div>
      ...
      <CardContent>
        {holdings.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-muted-foreground mb-4">No crypto holdings yet. Start tracking your portfolio!</p>
            <AddCryptoDialog onSuccess={onRefresh} />
          </div>
        ) : (
          <>
                  ...
                  <div key={holding._id} className="border border-border rounded-lg p-4 space-y-3 bg-accent/20">
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-3">
                        <CryptoLogo coinId={holding.coinId} symbol={holding.symbol} imageUrl={holding.imageUrl} size="sm" />
                        <div>
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm">
                      <div className="min-w-0">
                        ...
                        <p className="font-medium text-foreground truncate">
                          {holding.amount.toLocaleString('en-US', { minimumFractionDigits: 4, maximumFractionDigits: 8 })}
                        </p>
                      </div>
                      <div className="min-w-0">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border text-left">
                  ...
                  <th className="pb-3 text-xs font-medium text-muted-foreground uppercase tracking-wide">Exchange</th>
                  <th className="pb-3 text-xs font-medium text-muted-foreground uppercase tracking-wide text-right">Amount</th>
          ...
          {deleteDialog.holding && (
            <div className="my-4 rounded-lg border border-border bg-accent/30 p-4">
              <div className="flex items-center gap-3 mb-3">
                <CryptoLogo coinId={deleteDialog.holding.coinId} symbol={deleteDialog.holding.symbol} imageUrl={deleteDialog.holding.imageUrl} size="md" />
                <div>
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div>
                  ...
                  <span className="ml-2 font-medium">{deleteDialog.holding.amount}</span>
                </div>
                <div>
            ...
            <div className="my-4">
              <div className="rounded-lg border border-border bg-accent/30 p-4">
                <div className="flex items-center gap-3 mb-3 pb-3 border-b border-border">
                  <CryptoLogo coinId={notesDialog.holding.coinId} symbol={notesDialog.holding.symbol} imageUrl={notesDialog.holding.imageUrl} size="md" />
                  <div>
